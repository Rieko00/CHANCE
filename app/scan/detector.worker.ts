...
async function fetchModelWithCache(url: string): Promise<ArrayBuffer> {
  try {
    const cacheName = 'pulmocan-model-cache-v1';
...
function postprocess(
  ...
  // Auto-detect if coordinates are normalized (0..1) by checking if the first box 'cx' is < 2.0
  if (data[0 * numAnchors + 0] < 2.0) {
    isNormalized = true;
  }

  const scaleX = isNormalized ? origWidth : origWidth / INPUT_SIZE;
  const scaleY = isNormalized ? origHeight : origHeight / INPUT_SIZE;
