...
function drawDetections(
  ...
  detections.forEach((det) => {
    const color = CLASS_COLORS[det.class] ?? '#f59e0b';
    ...
    ctx.shadowBlur = 0;
    const label = `Nodule Mass ${(det.confidence * 100).toFixed(1)}%`;
    ...
    <div className="result-card">
      <div className="result-card-img">
        {displayUrl ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 200, color: 'var(--danger)', flexDirection: 'column', gap: 8, padding: 16 }}>
            <AlertCircle size={32} />
        ...
        {result.status === 'done' && result.detections !== undefined && (
          <div className="detection-list">
            {result.detections.length === 0 ? (
              <div className="no-detection">No nodule masses detected</div>
            ) : (
              result.detections.map((det, i) => (
                <div key={i} className="detection-item" style={{ borderLeftColor: 'var(--nodule-mass)' }}>
                  <div className="detection-dot" style={{ background: 'var(--nodule-mass)' }} />
                  <span className="detection-label">nodule_mass</span>
...
export default function ScanPage() {
  ...
  const stats = {
    ...
    nodule_mass: allDetections.filter((d) => d.class === 'nodule_mass').length,
  };
    ...
    <>
      <main className="main-content">
        ...
        <div className="page-header">
          <h1 className="page-title">CXR Scan Detection</h1>
          <p className="page-subtitle">Upload chest X-ray images to detect nodule masses using AI model.</p>
        </div>
        ...
        <div
          ...
          role="button"
          tabIndex={0}
          aria-label="Upload CXR scan images"
          onKeyDown={(e) => e.key === 'Enter' && fileInputRef.current?.click()}
        >
          <div className="drop-icon"><UploadCloud size={32} /></div>
          <div className="drop-title">
            {dragOver ? 'Drop here!' : mode === 'single' ? 'Drop a CXR scan or click to browse' : 'Drop CXR scans or click to browse'}
          </div>
          ...
          <div className="results-section">
            ...
            {allDetections.length > 0 && (
              <div className="summary-stats">
                ...
                <div className="stat-card" style={{ borderLeftColor: 'var(--nodule-mass)' }}>
                  <div className="stat-value" style={{ color: 'var(--nodule-mass)' }}>{stats.nodule_mass}</div>
                  <div className="stat-label">Nodule Mass</div>
                </div>
              </div>
            )}
        ...
        {files.length === 0 && (
          <div className="empty-state">
            ...
            <div className="empty-desc">Upload CXR scan images above to begin nodule detection</div>
          </div>
        )}
      </main>
